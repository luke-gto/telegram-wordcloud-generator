...
def load_json():
    easygui.msgbox(
        "Select the JSON file you got from the Telegram application",
        "Telegram chat wordcloud generator",
    )
    json_file_path = easygui.fileopenbox(
        msg="Select the JSON file", default=script_directory, filetypes="*.json"
    )
...
def tokenize_chat(jdata):
    ...
    easygui.msgbox(
        "Select the stopwords file. If you don't know what that is, please red the documentation",
        "Telegram chat wordcloud generator",
    )
    stopword_file = easygui.fileopenbox(
        msg="Select the stopwords file", default=script_directory, filetypes="*.txt"
    )
...
def standard_wordcloud(tokenized_chat):
    ...
    wc = WordCloud(background_color=color, width=1600, height=800).generate_from_text(
        tokenized_chat
    )
